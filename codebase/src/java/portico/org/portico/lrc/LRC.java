 ...
 * <p/>
 * Each LRC contains two {@link MessageSink}s. One for messages triggered by the local federate
 * and needing sending to the federation (the outgoing sink) and one for processing messages
 * received from other federates in the federation. Outgoing messages are passed directly to the
 * outgoing sink for processing. From here the handlers can process them, and if necessary, pass
 * them on to the connection for sending to the rest of the federation. When an
 * {@link IConnection} receives a message, it should first be put into a message queue (stored in
 * the LRCState). When the federate calls tick (resulting in one of the tick() methods being
 * called on the LRC), messages are extracted from the queue and passed into the incoming sink.
 * 
 * <p/>
 ...
 * just a marker interface and each HLA version interface should have its own implementaiton of
 * it. The typical process is to create a class that implements this interface, but to which you
 * can attached the HLA-version specific FederateAmbassador. An instance of this class is then
 * stored with the LRC (as the rather opaque ISpecHelper type). Message handlers that are used for
 ...
 * particular messages and turn them into FederateAmbassador callbacks (naturally, each is
 * specific to a particular HLA interface version). For example, callback handlers that work for
 * IEEE-1516 should provide the keyword defined in the static property
 * {@link PorticoConstants#KEYWORD_LRC13}. This way, when an LRC is created for 1516, only those
 * message handlers that define this keyword will be added to the incoming/outgoing sinks. If the
 * spec helper returns {@link HLAVersion#HLA13}, then the LRC will pass the keyword
...
public class LRC
{
	...
	public LRC( String policyFilePath )
	    throws Exception
	{
	...
	public void registerFederate( int federateHandle, String federateName )
	{
		...
		if( profile != null )
		{
			...
			System.out.println( "Federate " + federateName + " assigned to profile " +
			                    profile.getName() );
		}
		else
		{
	 ...
	 * 
	 * @param specHelper This allows an aribtrary piece of code to be attched to the LRC that can be
	 *                       used in callbacks that are specific to an HLA version. Each version has
	 *                       its own specification helper, stored in the LRC. The callback handlers
	 *                       for each HLA version are expected to supply the appropriate keywords to
	 *                       make sure that they only get applied when being attached to an LRC of
	 *                       the same version. They can then get the spec helper from the LRC and
	 *                       cast it to the proper type (thus giving them access to the proper
	 *                       FederateAmbassador).
	 */
	public LRC( ISpecHelper specHelper )
	...
	/**
	 * So that the messaging components can be properly reinitialized when a federate resigns from a
	 * federation and then joins another (the same LRC) this method contains all the logic to
	 * reinitialize the appropriate internal LRC components. This will wipe out the existing message
	 * sinks and reconfigure them. The connection will remain untouched.
	 */
	 ...
	 * LRC. It will also pass the appropriate component ID depending on which HLA interface is in
	 * use.
	 */
	private void initializeMessaging( Map<String,Object> initializationProperties ) throws JConfigurationException
	{
		...
		for( Module module : Container.instance().getHandlerRegistry().getAllModules() )
		{
			...
			Set<Class<?>> appliedHandlers =
			    module.apply( new MessageSink[]{ incoming, outgoing }, new String[]{ component },
	 ...
	 */
	private void initializeConnection( Map<String,Object> initializationProperties ) throws JConfigurationException
	{
		...
		{
			throw new JConfigurationException( "Can't create kernel: problem creating connection",
			                                   e );
		}
	...
	/**
	 * This method will look at the system property {@link PorticoConstants#PROPERTY_CONNECTION} see
	 * if a custom LRC connection has been specified. If one has not, the default implementation as
	 * defined in {@link PorticoConstants#CONNECTION_DEFAULT_IMPL} will be used. If one has, this
	...
	private Class<? extends IConnection> getConnectionImplementation() throws JConfigurationException
	{
		...
		{
			throw new JConfigurationException( "Problem locating connection class: " + property,
			                                   e );
		}
	}
	...
	//----------------------------------------------------------
	/**
	 * This should be called when a federate resigned from a federation. It will clear out all the
	 * existing federation state and reinitialize the messaging infrastructure. Note that this will
	 * have no effect on the connection.
	 */
	...
	/**
	 * Call this when the LRC is finished processing. It will cause the IConnection to disconnect
	 * from the federation.
	 */
	public void stopLrc()
	{
	 ...
	 */
	public void tick() throws JRTIinternalError,
	 ...
	 */
	public int tickUntilEmpty( long nanoWait ) throws JRTIinternalError,
	                                           JConcurrentAccessAttempted
	{
	 ...
	 */
	public boolean tick( double minSeconds, double maxSeconds ) throws JRTIinternalError,
	                                                            JConcurrentAccessAttempted
	{
	 ...
	 */
	public double tickFlush( double maxTime ) throws JRTIinternalError,
	                                          JConcurrentAccessAttempted
	{
		// don't process anything if callbacks aren't enabled
		if( state.areCallbacksEnabled() == false )
	 ...
	 * method will return. If there are no messages waiting for attention, wait at most as long we
	 * the provided wait time (in SECONDS) before giving up. Return <code>true</code> if there are
	 * more messages waiting to be processed, <code>false</code> if there are none left.
	 * 
	 ...
	 */
	public boolean tickSingle( double wait ) throws JRTIinternalError,
	                                         JConcurrentAccessAttempted
	{
		if( state.areCallbacksEnabled() == false )
	 ...
	 * called (although we'll extract callbacks via the same poll() call to ensure we only release
	 ...
	 * print a warning that there was no handler to process the message, but will not cause an error
	 * to be thrown. It will mark the context as having been successful so that incoming requests
	 * don't have to be handled if there is no need for them in a given component.
	 */
				...
				try
				{
					...
					if( message != null )
					{
						...
						catch( Exception e )
						{
							...
							logger.error( "Problem processing callback message: " + e.getMessage(),
							              e );
						}
					}
				}
